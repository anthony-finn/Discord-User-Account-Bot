...
const maximum_rank = process.env.MAXIUM_RANK;
...
function Authenticate(req, res, next_func) {
	if (req.body.auth_key === auth_key || req.method == "GET") {
		next_func();
  } else {
...
function CheckRank(request) {
	...
	return roblox.getRankInGroup(Group, TargetUser) // Check their rank
		.then(rank => {
			...
			if (rank > maximum_rank) { // Can't promote them because their rank is higher than our maximum_rank
				throw new Error('Original rank ' + rank + ' is above rank limit ' + maximum_rank);
			}
			return rank;
		});
}
