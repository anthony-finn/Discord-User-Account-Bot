...
function SetPoints() { // Validator for SetPoints
 	return [
		check('Targets')
			.custom((value, { req, loc, path }) => {
				if (typeof req.body.Targets === "object" && !!req.body.Targets) { return value; } else { throw new Error(`Parameter 'Targets' must be an object, not type '${typeof (value)}'`) }
			}),
		check('Amount')
			.custom((value, { req, loc, path }) => {
				if (typeof req.body.Amount === "number") { return value; } else { throw new Error(`Parameter 'Amount' must be an integer, not type '${typeof (value)}'`) }
			})
	] 
}
